...
def git_version():
    def _minimal_ext_cmd(cmd):
        ...
        out = subprocess.Popen(
            cmd, stdout=subprocess.PIPE, stderr=open(os.devnull, "w"), cwd=parentdir
        ).communicate()[0]
        return out
...
def runSubprocess(
    ...
    with process_handle(capture_output) as p_out, process_handle(
        capture_error
    ) as p_error, process_handle(in_file, mode="r") as p_in:
    ...
    try:
        ...
        if raise_not_exit:
            ...
            sys.exit(
                1
            )  # this should probably be replaced with raise(e) and more messages
    ...
    try:
        with open(os.devnull) as devnull:
            ...
            if not any(name in x for x in diff):
                subprocess.Popen(
                    [name], stdout=devnull, stderr=devnull, universal_newlines=True
                ).communicate()
            else:
                if name == "signalp":
                    subprocess.Popen(
...
def CheckDependencies(input):
    ...
    if missing != []:
        ...
        try:
            log.error(
                "Missing Dependencies: %s.  Please install missing dependencies and re-run script"
                % (error)
            )
        except NameError:
            sys.stderr.write(
...
def checkAugustusFunc():
    ...
    aug_cmd = [
        ...
        "--species=anidulans",
        profile,
        os.path.join(parentdir, "config", "busco_test.fa"),
    ]
    log.debug(" ".join(aug_cmd))
    proc = subprocess.Popen(
        aug_cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, universal_newlines=True
    )
    stdout, stderr = proc.communicate()
    stderr = stderr.strip()
    if isinstance(stdout, str):
            ...
            for line in infile:
                ...
                if line.startswith("#"):
                    ...
                    if len(cols) == 9:
                        contigs.add(cols[0])
                        outfile.write(
                            "{}\t{}\t{}".format(cols[0], newname, "\t".join(cols[2:]))
                        )
    return contigs
        ...
        while True:
            ...
            for x in results:
                try:
                    ...
                    remaining += 1
            sys.stdout.write(
                f"     Progress: {completed} complete, {failed} failed, {remaining} remaining        \r"
            )
...
def fix_busco_naming(busco_infile, genome, augustus, gffout, ploidy=1, proteins=False):
...
def convertgff2tbl(gff, prefix, fasta, prots, trans, tblout, external=False):
    ...
    for k, v in list(sortedGenes.items()):
        ...
        if locusTag not in renamedGenes:
...
def tblfilter(input, remove, output):
        ...
        with open(input, "r") as infile:
            for gene in readBlocks2(infile, ">Feature", "\tgene\n"):
                if gene[0].startswith(">Feature"):
                    ...
                    if locusTag and locusTag not in removeModels:
                        outfile.write("".join(gene))
                    else:
                        if not locusTag:
                            log.debug(
    ...
    log.debug(
        "Removed %i out of %i gene models from annotation"
    ...
    with open(input, "r") as all_annots:
        for line in all_annots:
            ...
            if refDB == "name" or refDB == "product":
                ...
                geneID = ID
            if geneID not in Annotations:
                Annotations[geneID] = {refDB: [description]}
            else:
                if refDB not in Annotations[geneID]:
                    Annotations[geneID][refDB] = [description]
                else:
            ...
            for line in custom_annots:
                ...
                if geneID not in Annotations:
                    ...
                    if refDB not in Annotations[geneID]:
                        ...
                        if "gene_synonym" not in Annotations[geneID]:
                    ...
                    if locusType in ["tRNA", "ncRNA", "rRNA"]:
                        ...
                        for line in gene:
                            if line.startswith("\t\t\tlocus_tag\t"):
                                ...
                                if "product" not in geneAnnot:
                                    outfile.write(line)
                            elif line.startswith("\t\t\ttranscript_id\t"):
                                ID = line.split("|")[-1]
                                ID = ID.split("_mrna")[0]
                                if ID not in transcriptsSeen:
        ...
        with open(output, "w") as outfile:
            with open(tmpoutput, "r") as infile:
                for line in infile:
                    ...
                    elif line.startswith("\t\t\ttranscript_id\t") or line.startswith(
                        "\t\t\tprotein_id\t"
                    ):
                        line = line.replace("|" + prefix, "|" + newtag)
                    outfile.write(line)
            ...
            with open(cdstrans, "w") as cdsout:
                for k, v in natsorted(list(input.items())):
                    ...
                    if v["type"] == "mRNA" and not v["CDS"]:
                        continue
                    if v["type"] == "mRNA" and not len(v["ids"]) == len(
                        v["mRNA"]
                    ) == len(v["CDS"]):
                        continue
                    for i, x in enumerate(v["ids"]):
...
def simpleFastaStats(fasta):
    ...
    n50_len = totalLen * 0.50
    n90_len = totalLen * 0.90
    n50 = None
    n90 = None
    runningSum = 0
...
def annotation_summary(
    ...
    if previous:  # load some stats that cant calculate from annotation
        ...
        try:
            stats["annotation"]["transcript-level"][
                "pct_exon_overlap_protein_evidence"
            ] = previousStats["annotation"]["transcript-level"][
                "pct_exon_overlap_protein_evidence"
            ]
        except KeyError:
            pass
        try:
            stats["annotation"]["transcript-level"][
                "pct_exon_overlap_transcript_evidence"
            ] = previousStats["annotation"]["transcript-level"][
                "pct_exon_overlap_transcript_evidence"
            ]
        except KeyError:
    ...
    if len(Genes) > 0:
        ...
        for k, v in Genes.items():
            ...
            for i in range(0, len(v["ids"])):
                if v["type"] == "mRNA":
                    ...
                    stats["annotation"]["transcript-level"]["total_exons"] += len(
                        v["mRNA"][i]
                    )
                    stats["annotation"]["transcript-level"]["total_exons"] += len(
                        v["5UTR"][i]
                    ...
                    stats["annotation"]["transcript-level"]["total_cds_exons"] += len(
                        v["CDS"][i]
                    )
                    if v["partialStart"][i] and v["partialStop"][i]:
                        stats["annotation"]["transcript-level"][
                            "CDS_no-start_no-stop"
                        ] += 1
                    ...
                    if v["go_terms"][i]:
                        stats["annotation"]["transcript-level"]["functional"][
                            "go_terms"
                        ] += 1
        ...
        stats["annotation"]["avg_gene_length"] = round(
            sum(geneLengths) / float(len(geneLengths)), 2
        )
...
def dicts2tbl(
        ...
        for k, v in natsorted(list(scaff2genes.items())):
            ...
            for genes in v:  # now loop through each gene on the scaffold
                ...
                if geneInfo["type"] == "mRNA" and not geneInfo["CDS"]:
                    ...
                    pseudo += 1
                    continue
                if geneInfo["type"] == "mRNA" and not len(geneInfo["ids"]) == len(
                    geneInfo["mRNA"]
                ) == len(geneInfo["CDS"]):
                    try:
                        log.debug(
                            "Incompatible annotation found: {:}\n{:}".format(
                                genes, geneInfo
                            )
                        )
                    except NameError:
    ...
    if any(i > 0 for i in [duplicates, pseudo, nocds]):
        try:
            print(
                (
                    "Skipped {:,} annotations: {:,} pseudo genes; {:,} no CDS; {:,} duplicated features".format(
...
def getGBKinfo(input):
    ...
    with open(input, "r") as infile:
        for record in SeqIO.parse(infile, "genbank"):
            ...
            try:
                organism = (
                    record.annotations["organism"].replace("Unclassified.", "").rstrip()
                )
            except KeyError:
                pass
            try:
...
def dict2proteins(input, prots):
    with open(prots, "w") as protout:
        for k, v in natsorted(list(input.items())):
            ...
            if v["type"] == "mRNA" and not v["CDS"]:
                continue
            if v["type"] == "mRNA" and not len(v["ids"]) == len(v["mRNA"]) == len(
                v["CDS"]
            ):
                continue
            for i, x in enumerate(v["ids"]):
...
def gb_feature_add2dict(f, record, genes):
    ...
    if (
        ID and "-T" in ID
    ):  # then this is from funannotate, okay to modify - this is to capture apparent tbl2asn local error
    ...
    if f.type == "gene":
        ...
        if not ID:
            try:
                log.info(
                    "putative transcript from %s has no ID\n(%s %s %s)"
        ...
        for k, v in natsorted(list(Genes.items())):
            if v["strand"] == "+":
                ...
                for i, x in enumerate(sortedCDS):  # loop through tuples
                    output.write(
                        "{:}\txnt2h\tCDSpart\t{:}\t{:}\t.\t{:}\t.\tsrc=XNT;grp={:};pri=4\n".format(
                    ...
                    if len(sortedCDS) > 1:
                        try:
                            output.write(
                                "{:}\txnt2h\tintron\t{:}\t{:}\t.\t{:}\t.\tsrc=XNT;grp={:};pri=4\n".format(
                                    v["contig"],
                                    x[1] + 1,
                                    sortedCDS[i + 1][0] - 1,
                                    v["strand"],
        ...
        for line in infile:
            ...
            if ID not in Genes:
                ...
                if contig != Genes[ID]["contig"]:
                    log.debug(
                        "ERROR: {:} mapped to multiple contigs: {:} and {:}".format(
                            ID, contig, Genes[ID]["contig"]
                        )
                    )
                    continue
...
def harmonize_transcripts(
    ...
    log.info(
        "Parsed {:,} transcript alignments from: {:}".format(len(Genes), alignments)
    )
    if evidence:  # if nothing here then just move on
        ...
        if seqcount > 0:
            log.info(
                "Aligning {:,} unique transcripts [not found in exising alignments] with minimap2".format(
                    seqcount
                )
            )
    ...
    for k, v in list(Genes.items()):
        for i in range(0, len(v["ids"])):
            ...
            if v["type"] in ["mRNA", "tRNA", "ncRNA", "rRNA"]:
                ...
                mrnaSeq = getSeqRegions(SeqRecords, v["contig"], sortedExons)
                if gap_filter:
                    mrnaSeq, Genes[k]["mRNA"][i] = start_end_gap(
                        mrnaSeq, Genes[k]["mRNA"][i]
                    )
            ...
            if v["type"] == "mRNA":
                ...
                try:
                    currentphase = v["phase"][i]  # this is never used below?
                except IndexError:
                    pass
                if (
                    "?" in v["phase"][i]
                ...
                if codon_start > 1:
                    if v["strand"] == "+":
                        ...
                        sys.stderr.write(
                            f"ERROR nonsensical strand ({v['strand']}) for gene {k}\n"
                        )
                Genes[k]["cds_transcript"].append(cdsSeq)
                Genes[k]["CDS"][i] = sortedCDS
            ...
            for i in range(0, len(v["ids"])):
                ...
                if v["type"] == "mRNA":
                    ...
                    for y in range(0, num_cds):
                        ...
                        current_phase = (
                            current_phase
                            - (int(v["CDS"][i][y][1]) - int(v["CDS"][i][y][0]) + 1)
                        ) % 3
                        if current_phase == 3:
                            current_phase = 0
        ...
        with open(input, "r") as infile:
            for line in infile:
                ...
                if feature == "gene":
                    ...
                    # should we use f-strings here instead for easier to follow, or a join on tab?
                    outfile.write(
                        "{:},\tID={:}.exon{:};Parent={:};\n".format(
                            "\t".join(
                                [contig, source, "exon", start, end, ".", strand, "."]
                            ),
                            transID,
                            num,
                            transID,
...
def runCodingQuarry(genome, stringtie, cpus, output):
    ...
    cmd = [
        ...
        str(cpus),
        "-f",
        os.path.realpath(genome),
        "-t",
        os.path.realpath(stringtieGFF3),
    ]
...
def minimap2Align(transcripts, genome, cpus, intron, output):
    ...
    p2 = subprocess.Popen(
        samtools_cmd, stdout=subprocess.PIPE, stderr=FNULL, stdin=p1.stdout
    )
    p1.stdout.close()
    p2.communicate()
...
def runGMAP(transcripts, genome, cpus, intron, tmpdir, output):
    ...
    with open(build_log, "w") as logfile:
        subprocess.call(
            ["gmap_build", "-D", tmpdir, "-d", "genome", "-k", "13", genome],
            stdout=logfile,
            stderr=logfile,
        )
    # now map transcripts
...
def runBUSCOv5(
    ...
    busco_data = os.path.join(
        tmpdir, runDir, "run_{}_odb10".format(lineage), "full_table.tsv"
    )
    runSubprocess(cmd, tmpdir, log)
...
def runBUSCO(input, Database, cpus, tmpdir, output):
    ...
    if sys.version_info > (3, 0):
        ...
        BUSCO = os.path.join(parentdir, "aux_scripts", "funannotate-BUSCO2-py2.py")
    cmd = [
        BUSCO,
        "-i",
        input,
    ...
    with open(output, "w") as out:
        with open(
            os.path.join(tmpdir, "run_busco", "full_table_busco.tsv"), "r"
        ) as busco:
            for line in busco:
                if line.startswith("#"):
                    continue
        ...
        with open(input, "r") as infile:
            for line in infile:
                ...
                if cols[Genei] != "":
                    if (
                        "_" not in cols[Genei]
                        and "." not in cols[Genei]
                        and number_present(cols[Genei])
                    ):
                        Gene = cols[Genei]
...
def parseSignalP(sigP, secretome_annot):
    ...
    with open(sigP, "r") as results:
        for line in results:
            ...
            if version < 5:
                ...
                version == 5
            ):  # version 5 has different format and tab delimited hooray!
                if "\t" in line:
                    cols = line.split("\t")
    ...
    with open(secretome_annot, "w") as secout:
        for k, v in natsorted(list(sigpDict.items())):
            if v[1] != "":
                secout.write(
                    "{:}\tnote\tSECRETED:SignalP(1-{:},cutsite={:},prob={:})\n".format(
                        k, v[0], v[1], v[2]
                    )
                )
            else:
...
def RunGeneMarkES(
    ...
    try:
        ...
        log.error(
            "GeneMark-ES failed: {:} file missing, please check logfiles.".format(
                os.path.join(outdir, "output", "gmhmm.mod")
            )
        )
...
def RunGeneMarkET(
    ...
    try:
        ...
        log.error(
            "GeneMark-ET failed: {:} file missing, please check logfiles.".format(
                os.path.join(outdir, "output", "gmhmm.mod")
            )
        )
    # convert genemark gtf to gff3 so GAG can interpret it
...
def runGlimmerHMM(fasta, gff3, dir, output):
    ...
    runSubprocess(
        cmd, ".", log, only_failed=True
    )  # only_error=True --> stdout/stderr to devnull unless error
...
def runSnap(fasta, gff3, minintron, maxintron, dir, output):
    ...
    if not checkannotations(snapRaw):
        ...
        cmd = [
            "fathom",
            os.path.abspath(origzff),
            os.path.abspath(trainingFasta),
            "-categorize",
            "1000",
            "-min-intron",
            str(minintron),
...
def SystemInfo():
    ...
    log.info(
        "OS: %s, %i cores, ~ %i GB RAM. Python: %s"
...
def runtRNAscan(input, tmpdir, output, cpus=1, precalc=False):
    ...
    if not checkannotations(tRNAout):
        log.info(
            "tRNAscan-SE seems to have failed, check logfile for error. You can pass precalculated results to --trnascan"
        )
        return False
                        ...
                        if not matchLine:
                            ...
                            if "\tgene\t" in line:
                                ...
                                if bad_reason:
                                    ...
                                    log.debug(
                                        "%s was removed in removeBadModels function for unknown reason, please check manually"
                                        % bad_ID
                                    )
                                    line = line.replace(
...
def RemoveBadModels(
    proteins, gff, length, repeats, BlastResults, tmpdir, methods, output
):
    reason = {}
    tooShort = 0
    repeat = 0
    ...
    if numTotal > 0:
        log.info(
            "Found {:,} gene models to remove: {:,} too short; {:,} span gaps; {:,} transposable elements".format(
...
def ParseErrorReport(input, Errsummary, val, Discrep, output, keep_stops):
    ...
    with open(Errsummary) as summary:
        for line in summary:
            if "ERROR" in line:
                ...
                if (
                    "SEQ_DESCR.OrganismIsUndefinedSpecies" in line
                    or "SEQ_DESCR.BadOrgMod" in line
                    or "SEQ_FEAT.MissingTrnaAA" in line
                    or "SEQ_INST.TerminalNs" in line
                ):
                    pass
    ...
    with open(Discrep, "r") as discrep:
        ...
        for block in readBlocks(discrep, "DiscRep_"):
            if (
                "DiscRep_ALL:OVERLAPPING_GENES::" in block[0]
                or "DiscRep_SUB:RNA_CDS_OVERLAP::" in block[0]
            ):
                for item in block:
        ...
        for rec in SeqIO.parse(infile, "genbank"):
            if "structured_comment" in rec.annotations:
                if "antiSMASH-Data" in rec.annotations["structured_comment"]:
                    version = int(
                        rec.annotations["structured_comment"]["antiSMASH-Data"][
                            "Version"
                        ].split(".")[0]
                    )
            break
...
def ParseAntiSmash(input, tmpdir, output, annotations):
    ...
    log.info(
        "Now parsing antiSMASH v{:} results, finding SM clusters".format(smash_version)
    )
    BackBone = {}
    SMCOGs = {}
    bbSubType = {}
            ...
            for rec_num, record in enumerate(SeqRecords):
                for f in record.features:
                    ...
                    if smash_version < 6:
                        ...
                        if "_" in record.id:
                            try:
                                numericalContig = "{}_{}".format(
                                    baseName, int(record.id.rsplit("_", 1)[-1])
                                )
                            except ValueError:
                                if "." in record.id:
                                    try:
                                        ...
                                        numericalContig = "{}_{}".format(
                                            baseName, int(record.id.rsplit(".", 1)[-1])
                                        )
                        else:  # just get the numbers
                            numericalContig = "{}_{}".format(
                                baseName, int("".join(filter(str.isdigit, record.id)))
                            )
                    else:
                    ...
                    if smash_version == 4:
                        if f.type == "cluster":
                            ...
                            clusternum = f.qualifiers.get("note")[0].replace(
                                "Cluster number: ", ""
                            )
                            antibed.write(
                                "%s\t%s\t%s\tCluster_%s\t0\t+\n"
    ...
    # if smash_version == 4:
    log.info(
        "Found %i clusters, %i biosynthetic enyzmes, and %i smCOGs predicted by antiSMASH"
    ...
    for k, v in natsorted(Genes.items()):
        if v["type"] == "mRNA":
            if v["location"] in interClust[v["contig"]]:
                ...
                if clusterName not in dictClusters:
...
def genomeStats(input):
    ...
    return [
        ...
        uniqueIso,
        locus_tag,
        "{0:,}".format(GenomeSize) + " bp",
        "{0:,}".format(LargestContig) + " bp",
        "{0:,}".format(AvgContig) + " bp",
        "{0:,}".format(ContigNum),
        "{0:,}".format(N50) + " bp",
        "{:.2f}".format(pctGC) + "%",
        "{0:,}".format(Genes),
            ...
            for i in range(0, len(v["ids"])):
                ...
                if v["type"] == "mRNA":
                    ...
                    try:
                        ...
                        Protein = ""
                        print(
                            "ERROR: No amino acid sequence exists for {}".format(
                                v["ids"][i]
                            )
                        )
                else:
                ...
                if v["strand"] == "-":
                    ...
                    Transcript = RevComp(Transcript)
                    CDSTranscript = RevComp(CDSTranscript)
                result += [";".join(therest), gDNA, Transcript, CDSTranscript, Protein]
                            ...
                            for i in notes:
                                if i.startswith("GO"):
                                    ...
                                    if hit in meropsDict:
                                        ...
                                        log.error(
                                            "MEROPS database inconsistency: %s not found"
                                            % hit
                                        )
                                elif i.startswith("CAZy:"):
                                    hit = i.replace("CAZy:", "")
...
def drawStackedBar(panda, type, labels, ymax, output, colors=False):
    ...
    # labels
    # d_labels = panda.index.values   # this isn't used below
    # y-ticks
    ...
    # colors
    if not colors:
        color_palette = sns.hls_palette(len(panda.columns), l=0.4, s=0.8).as_hex()
    ...
    for column in panda.columns:
        legends.append(
            mpatches.Patch(
                ...
                label=panda.columns.values[i]
                + ": "
                + labels.get(panda.columns.values[i]),
            )
        )
        i += 1
    lgd = ax.legend(
...
def donutplot(df, LongName, output, colors=False):
    with warnings.catch_warnings():
        ...
        import matplotlib.pyplot as plt
        import matplotlib.patches as mpatches
        import seaborn as sns  # this is reportedly not used
    # create data
    longnames = []
    ...
    # get size of table
    categories = len(df.columns)  # this is reportedly not used
    total = len(df.index)
    Rows = total // 2
    Rows += total % 2
    ...
    for k in range(total):
        ...
        plt.title(species[k])
    patches = [
        mpatches.Patch(color=color_palette[i], label="{:s}".format(longnames[i]))
...
def drawbarplot(df, output):
    ...
    if len(df) > len(resources.pref_colors):
        colorplot = sns.husl_palette(len(df), l=0.5).as_hex()
...
def ortho2phylogeny(
    ...
    if len(df) < int(num):
        number = len(df)
        log.info(
            "Found %i single copy BUSCO orthologs, will use all to infer phylogeny"
            % (len(df))
        )
        subsampled = df
    else:
        number = int(num)
        log.info(
            "Found %i single copy BUSCO orthologs, will randomly select %i to infer phylogeny"
            % (len(df), number)
        )
    ...
    cmd = [
        ...
        "--anysymbol",
        "--quiet",
        os.path.join(tmpdir, "phylogeny.concat.fa"),
    ]
    runSubprocess(
        cmd, ".", log, capture_output=os.path.join(tmpdir, "phylogeny.mafft.fa")
    )
    cmd = [
        "trimal",
        "-in",
        os.path.join(tmpdir, "phylogeny.mafft.fa"),
        "-out",
        os.path.join(tmpdir, "phylogeny.trimal.phylip"),
        "-automated1",
        "-phylip",
    ]
    runSubprocess(cmd, ".", log)
    if ml_method == "raxml":
        ...
        pylab.axis("off")
        pylab.savefig(
            os.path.join(tmpdir, "ML.phylogeny.pdf"),
            format="pdf",
            bbox_inches="tight",
            dpi=1000,
        )
...
def selectTrainingModels(input, fasta, genemark_gtf, output, tmpdir):
    ...
    log.debug(
        "{:,} PASA genes; {:,} have multi-CDS; {:,} from filterGeneMark; {:,} have multi-CDS".format(
    ...
    finalIgnoreList = []
    for x in ignoreList:
        if x not in finalIgnoreList:
            finalIgnoreList.append(x)
    for y in blastignore:
        if y not in finalIgnoreList:
    ...
    with open(aug_log, "w") as logfile:
        if not CheckAugustusSpecies(train_species):
            subprocess.call(
                [
                    NEW_SPECIES,
                    "--AUGUSTUS_CONFIG_PATH={:}".format(config_path),
                    species,
                ],
                stdout=logfile,
                stderr=logfile,
        ...
        subprocess.call(
            [RANDOMSPLIT, TrainSet, str(num_training)],
            cwd=os.path.join(outdir, "predict_misc"),
        )
        if os.path.isfile(os.path.join(outdir, "predict_misc", TrainSet + ".train")):
            with open(
                os.path.join(outdir, "predict_misc", "augustus.initial.training.txt"),
                "w",
            ) as initialtraining:
                subprocess.call(
                    [
            ...
            if optimize:
                ...
                p2.communicate()
                with open(
                    os.path.join(outdir, "predict_misc", "augustus.final.training.txt"),
                    "w",
                ) as finaltraining:
                    subprocess.call(
                        [
...
def alignMAFFT(input, output):
    ...
    with open(output, "w") as outfile:
        subprocess.call(
            ["mafft", "--anysymbol", "--quiet", input], stderr=FNULL, stdout=outfile
        )
...
def align2Codon(alignment, transcripts, output):
    ...
    with open(output, "w") as outfile:
        subprocess.call(
            [
                "perl",
                os.path.join(parentdir, "aux_scripts", "pal2nal.pl"),
                alignment,
                transcripts,
                "-output",
                "fasta",
